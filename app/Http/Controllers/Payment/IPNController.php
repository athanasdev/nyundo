...
class IPNController extends Controller
{
    ...
    public function handle(Request $request)
    {
        // ... (The HMAC signature validation code at the top remains the same)
        $ipnSecret = env('NOWPAYMENTS_IPN_SECRET');
        $receivedHmac = $request->header('x-nowpayments-sig');
        ...
        $requestJson = $request->getContent();
        $hmac = hash_hmac("sha512", $requestJson, trim($ipnSecret));
        ...
        try {
            $payment = Payment::updateOrCreate(
                ...
                [
                    ...
                    'pay_amount' => $requestData['pay_amount'],
                    'pay_currency' => $requestData['pay_currency'],
                    'amount_received' => $requestData['actually_paid'] ?? $requestData['amount_received'] ?? 0,
                    'pay_address' => $requestData['pay_address'] ?? null,
                    'network' => $requestData['network'] ?? null,
                    'payment_created_at' => Carbon::parse($requestData['created_at'] ?? now()),
                    'payment_updated_at' => Carbon::parse($requestData['updated_at'] ?? now()),
        ...
        |--------------------------------------------------------------------------
        |
        | This condition is temporarily set to 'waiting' for testing purposes.
        | YOU MUST CHANGE THIS TO 'finished' BEFORE DEPLOYING TO PRODUCTION.
        | Failure to do so will result in crediting users for incomplete payments.
        |
        */
            if ($requestData['payment_status'] === 'waiting' && $payment->user_id) {
                ...
                if ($user) {
                    $depositAmount = $requestData['actually_paid'] ?? $payment->pay_amount;
                    $bonusRate = 0.01; // 1% bonus
                    $bonusAmount = $depositAmount * $bonusRate;
                    ...
                    Transaction::create([
                        ...
                        'balance_before' => $balanceBefore,
                        'balance_after'  => $balanceAfterDeposit,
                        'description'    => 'Deposit via NOWPayments ID: ' . $payment->payment_id,
                    ]);
            ...
            DB::rollBack();
            Log::error('IPN Handling failed: ' . $e->getMessage() . ' on line ' . $e->getLine());
            return response()->json(['error' => 'Internal Server Error'], 500);
        }
    }
