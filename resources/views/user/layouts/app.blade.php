    ...
    <style>
        * {
        ...
        .container-main {
            ...
            padding-top: 150px; /* Approx 64px for header + 70px for profile card + margin */
        ...
        .header {
            ...
            align-items: center;
            margin-bottom: 0; /* Removed margin-bottom as profile card is separate */
            padding: 10px 20px; /* Adjusted padding */
            background: #1e2329;
            border-bottom: 1px solid #2b3139; /* Added border-bottom to fixed header */
            position: fixed; /* Make header fixed */
            top: 0;
            left: 0;
            right: 0;
            z-index: 1002; /* Ensure header is above other content */
            height: 64px; /* Define a fixed height for the header */
        }
        ...
        .logo-link { /* For the logo to be a link */
            text-decoration: none;
            display: flex;
            align-items: center;
            color: #f0b90b; /* Original .logo color */
        }

        .header-logo-img { /* For an image logo */
            height: 30px; /* Adjust as needed */
            width: auto;
            /* margin-right: 8px; /* Space between img and text if any - removed as logo-text is part of fallback */
        }
        .logo-text { /* For text part of the logo if image fails or if no image */
            font-size: 20px; /* Original .logo font-size */
            font-weight: 600; /* Original .logo font-weight */
            color: #f0b90b;
            margin-left: 8px; /* Add space if text is next to icon */
        }
        .logo-link .fas.fa-chart-line { /* Fallback icon style if image fails */
             font-size: 24px; /* Original .logo i font-size */
             /* margin-right: 8px; Replaced by gap on .logo-link or direct margin on .logo-text */
             color: #f0b90b;
        }


        .user-balance-display { /* Specific class for balance in header */
            display: flex;
            align-items: center;
            gap: 16px; /* Original .user-balance gap */
        }
        ...
        .balance-amount {
            font-size: 14px; /* Adjusted for header context */
            font-weight: 600;
            margin-bottom: 1px; /* Adjusted for header */
        }
        .balance-label {
            color: #848e9c;
            font-size: 10px; /* Adjusted for header context */
        }
        /* --- End Main Header --- */
        ...
        .header-action-btn {
            padding: 6px 12px;
            border: 1px solid #4a4a4a; /* Softer border */
            background: #2b3139; /* Darker button background */
            color: #c1c8d1; /* Lighter text */
            border-radius: 4px;
            font-weight: 500;
            cursor: pointer;
        ...
        .header-action-btn i { font-size: 13px; }
        .header-action-btn:hover {
            border-color: #f0b90b;
            color: #f0b90b;
            background-color: #363d47;
        }
        .header-action-btn.logout { /* Specific style for logout button */
            border-color: #f6465d;
            color: #f6465d;
            background-color: transparent; /* Make it more like an outline button initially */
        }
        .header-action-btn.logout:hover {
            background-color: #f6465d;
            color: #fff;
        }
        .logout-form-inline { /* For the form wrapping the logout button */
            display: inline;
            margin: 0;
            padding: 0;
        }
        ...
        .user-profile-card {
            background: #1e2329; /* Consistent with header/card background */
            border-radius: 6px;
            border: 1px solid #2b3139;
            padding: 15px 20px;
            margin-bottom: 20px; /* Space below this card before main content */
            display: flex;
            align-items: center;
            gap: 15px;
        ...
        .profile-image-placeholder {
            ...
            height: 50px;
            border-radius: 50%;
            background-color: #2b3139; /* Dark placeholder background */
            display: flex;
            align-items: center;
            justify-content: center;
            overflow: hidden; /* If you use an actual img tag */
            flex-shrink: 0;
        }
        .profile-image-placeholder i { /* For Font Awesome icon fallback */
            font-size: 24px;
            color: #565f6b; /* Muted icon color */
        }
        .profile-image-placeholder img { /* If you use an actual image for profile */
            width: 100%;
            height: 100%;
            object-fit: cover;
        }
        ...
        .profile-username {
            font-size: 1.1em;
            font-weight: 600;
            color: #eaecef; /* Primary text color */
            margin-bottom: 3px;
        }
        .profile-unique-id {
            font-size: 0.8em;
            color: #848e9c; /* Secondary text color */
            background-color: #2b3139; /* Darker chip background */
            padding: 2px 5px;
            border-radius: 3px;
        ...
        .card-header {
            padding: 16px 20px; /* Kept your original card header padding */
        ...
        .card-title i { color: #f0b90b !important; } /* Ensure card title icons are accent color */

        .card-body {
            padding: 16px 20px; /* Kept your original card body padding */
        }
        .card-body.no-padding { padding: 0; }

        .coins-list { display: flex; flex-direction: column; }
        .coin-item { display: flex; align-items: center; justify-content: space-between; padding: 12px 20px; border-bottom: 1px solid #2b3139; transition: background-color 0.2s; cursor: pointer; }
        ...
        .coin-icon { width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 600; color: white; }
        .btc { background: #f7931a; } .eth { background: #627eea; } .bnb { background: #f0b90b; } .ada { background: #0033ad; } .sol { background: #9945ff; } .dot { background: #e6007a; }
        .xrp { background: #0077c8; } .doge { background: #c3a634; }
        .avax { background: #e84142; } .link { background: #2a5ada; }
        .ltc { background: #bebebe; } .trx { background: #eb0029; }
        .shib { background: #ffc107; } .uni { background: #ff007a; }
        .bch { background: #8dc351; } .icp { background: #29abe2; }
        .xlm { background: #000000; } .atom { background: #5064fb; }
        ...
        .chart-container { height: 300px; background: #0b0e11; border-radius: 4px; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; }
        .trading-view { width: 100%; height: 100%; background: linear-gradient(135deg, rgba(15, 185, 177, 0.05), rgba(240, 185, 11, 0.05)); position: relative; display: flex; align-items: center; justify-content: center; }
        .chart-placeholder { color: #848e9c; font-size: 16px; display: flex; align-items: center; gap: 8px;}
        .quick-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        .action-btn { padding: 12px 16px; border: none; border-radius: 4px; font-weight: 500; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px; font-size: 14px; }
        ...
        .portfolio-item { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid #2b3139; }
        .portfolio-item:last-child { border-bottom: none; }
        .bot-status { display: flex; align-items: center; justify-content: space-between; padding: 16px; background: rgba(14, 203, 129, 0.1); border-radius: 4px; margin-bottom: 16px; border: 1px solid rgba(14, 203, 129, 0.2); }
        ...
        .signal-item.buy { border-left-color: #0ecb81; } .signal-item.buy .signal-type i { color: #0ecb81; }
        .signal-item.sell { border-left-color: #f6465d; } .signal-item.sell .signal-type i { color: #f6465d; }
        .signal-info { flex: 1; }
        ...
        .activity-item { display: flex; align-items: center; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid #2b3139; }
        ...
        .bottom-nav {
            ...
            padding: 8px 0; z-index: 1000; box-shadow: 0 -2px 5px rgba(0,0,0,0.1);
            height: 60px;
        }
        .nav-container {
        ...
        .nav-item {
            ...
            cursor: pointer; transition: all 0.2s ease; padding: 4px 8px;
        ...
        @media (max-width: 768px) {
            .container-main { padding: 10px; padding-top: 135px; /* Increased for stacked header + profile */ padding-bottom: 75px; }
            .header { flex-direction: column; height: auto; gap: 10px; padding: 10px; }
            .header-left { flex-direction: column; gap: 10px; width: 100%;}
            .user-balance-display { justify-content: space-around; width: 100%; margin-top: 10px;}
            .header-actions { margin-top: 10px; width: 100%; justify-content: space-around;}
            .user-profile-card { margin-top: 5px; flex-direction: column; text-align:center; } /* Profile card stacks */
         ...
         @media (max-width: 480px) {
            .container-main { padding-top: 150px; } /* Further adjust for very small screens */
            ...
            .header-action-btn { padding: 6px 12px; font-size: 12px; }
            .logo-text { font-size: 18px;}
            .header-logo-img { height: 28px;}
         }
         /* Alert styles (from original) */
        .alert { padding: 12px 15px; margin-bottom: 20px; border-radius: 4px; font-size: 0.9em; border: 1px solid transparent; }
    ...
    <header class="header">
        <div class="header-left">
            <a href="{{ route('dashboard') }}" class="logo-link">
                ...
                <span class="logo-text" style="display:none;"><i class="fas fa-chart-line icon-fallback"></i> CoinTrades</span>
            </a>
        ...
        <div class="header-actions">
            ...
            <a href="#" id="withdrawButton" class="header-action-btn">
