...
<style>
    ...
    .coins-table {
        ...
        color: #eaecef; /* Ensure text is visible */
    }
    .coins-table th, .coins-table td {
        padding: 10px 8px; /* Adjusted padding */
        text-align: left;
        border-bottom: 1px solid #2b3139; /* Match card border */
        font-size: 0.9em;
    }
    .coins-table th {
        color: #848e9c;
    ...
    .coins-table .coin-market-info img {
        ...
        width: 28px;
        background: #fff; /* If icons have transparent parts */
        border-radius: 50%;
    }
    ...
    .coins-table .percent-col svg { /* Ensure SVGs are sized correctly */
        width: 14px;
        height: 14px;
        vertical-align: middle;
    }
    .coins-table .success-color span { color: #0ecb81; }
    .coins-table .error-color span { color: #f6465d; }

    /* Override for card-body padding if table is direct child */
    .card-body.no-padding {
        padding: 0;
    }
    .coins-list .coin-item { /* Keep original coin item styles if used elsewhere or as fallback */
                ...
                <div class="card-body no-padding"> {{-- Remove padding if table touches edges --}}
                    <table class="coins-table" id="realtimeCoinsTable">
                        ...
                        <tbody id="coinsTableBody">
                            ...
                            <tr><td colspan="4" style="text-align:center; padding: 20px; color: #848e9c;">Connecting to live market data...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="quick-actions">
                        ...
                        <a href="{{route('withdraw')}}" class="action-btn sell">
...
<script>
    ...
    let assetPagePortfolioData = [
        { symbol: 'BTC', amount: 0.5, value: 0, avgBuyPrice: 40000 },
        { symbol: 'ETH', amount: 2.5, value: 0, avgBuyPrice: 2800 },
        { symbol: 'BNB', amount: 10, value: 0, avgBuyPrice: 300 }
    ];
    ...
    function renderPortfolio() {
        ...
        portfolioListEl.innerHTML = assetPagePortfolioData.map(item => {
            ...
            const currentPrice = currentCoinLive ? parseFloat(currentCoinLive.price) : (item.value / (item.amount || 1));

            item.value = currentPrice * item.amount;
            totalPortfolioValue += item.value;

            const pnl = (currentPrice - item.avgBuyPrice) * item.amount;
            const pnlPercentage = (item.avgBuyPrice * item.amount) !== 0 ? (pnl / (item.avgBuyPrice * item.amount)) * 100 : 0;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        ws.onopen = () => {
            ...
            if(coinsTableBodyEl.firstChild && coinsTableBodyEl.firstChild.textContent.includes("Connecting")) {
        ...
        ws.onmessage = (event) => {
            ...
            updateElementsByClass(`${symbolBase}-price`, `${formatCurrency(price, 2, 4)} USDT`); // Show more precision for price
            ...
            updateElementsByClass(`${symbolBase}-volume`, Number(volume).toLocaleString('en-US', {maximumFractionDigits: 0}));
        ...
        ws.onerror = (error) => {
            ...
            if(coinsTableBodyEl) coinsTableBodyEl.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 20px; color: #f6465d;">Error connecting to live market data.</td></tr>`;
        };

        ws.onclose = () => {
            console.warn("WebSocket Closed. Attempting to reload page in 3 seconds to reconnect...");
            if(coinsTableBodyEl) coinsTableBodyEl.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 20px; color: #f0b90b;">Connection lost. Reconnecting...</td></tr>`;
