...
<style>
    .withdraw-panel-card {
        max-width: 650px; /* Slightly wider for more details */
        margin-left: auto;
        margin-right: auto;
        margin-top: 20px;
    }
    ...
    .alert-error {
        color: #f6465d; background-color: rgba(246, 70, 93, 0.1); border: 1px solid rgba(246, 70, 93, 0.3);
    }
    ...
    .alert-success {
        color: #0ecb81; background-color: rgba(14, 203, 129, 0.1); border: 1px solid rgba(14, 203, 129, 0.3);
    }
    ...
    .form-group input[type="text"], /* For read-only address */
    .form-group select {
        width: 100%;
        padding: 12px 15px;
        border-radius: 4px;
    ...
    .form-group input[readonly] {
        background-color: #222531; /* Slightly different for readonly */
        cursor: default;
    }
    ...
    .copy-btn-sm {
        padding: 10px 15px; /* Match input height better */
        background: #2b3139;
        color: #f0b90b;
    ...
    .copy-btn-sm:hover { background: #363d47; }
    .copy-btn-sm.copied { background: #0ecb81; color: #fff; border-color: #0ecb81;}


    /* Calculation Summary Box */
    .withdrawal-summary {
        background-color: #222531; /* Darker background for summary */
        border: 1px solid #2b3139;
        border-radius: 4px;
        padding: 15px;
        margin-top: 10px; /* Space above the summary if amount is entered */
        margin-bottom: 20px;
        font-size: 0.9em;
    }
    ...
    .withdrawal-summary span.value {
        font-weight: 600;
        color: #f0b90b; /* Accent for values */
    }
    .withdrawal-summary span.fee-value {
        color: #f6465d; /* Red for fee */
    }
    .withdrawal-summary span.receivable-value {
        color: #0ecb81; /* Green for receivable */
        font-size: 1.1em; /* Make receivable slightly larger */
    }

    .submit-btn { /* Consistent with deposit page */
        width: 100%;
        padding: 12px 15px;
    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('withdraw.request') }}"> {{-- Define this route --}}
            ...
            <div class="form-group">
                ...
                <input type="number" id="amount_to_withdraw" name="amount"
                       value="{{ old('amount') }}"
                       step="0.01"
                       min="10" {{-- Example minimum withdrawal --}}
            ...
            <div class="withdrawal-summary" id="withdrawalSummaryBox" style="display: none;">
                <p><strong>Withdrawal Amount:</strong> <span class="value">$<span id="summaryAmount">0.00</span></span></p>
                <p><strong>Withdrawal Fee (5%):</strong> <span class="fee-value">$<span id="summaryFee">0.00</span></span></p>
                <hr style="border-color: #2b3139; margin: 5px 0;">
                <p><strong>You Will Receive:</strong> <span class="receivable-value">$<span id="summaryReceivable">0.00</span> USDT</span></p>
            </div>

            <div class="form-group">
                <label for="security_pin">Withdrawal PIN / Security Password:</label>
                <input type="password" id="security_pin" name="security_pin" placeholder="Enter your PIN/Password" required>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const withdrawalAddressInput = document.getElementById('withdrawal_address_display') as HTMLInputElement | null;
    ...
    if (amountInput) {
        ...
        amountInput.addEventListener('blur', function() { // Format to 2 decimal places on blur
            ...
            if (!isNaN(value)) {
                ...
                if (!isNaN(minValue) && value < minValue && this.value !== "") { // only apply min if not empty
                    value = minValue;
                }
    ...
    if (copyWithdrawalAddressBtn && withdrawalAddressInput && copyWithdrawalFeedback) {
        copyWithdrawalAddressBtn.addEventListener('click', function() {
            if (!withdrawalAddressInput.value || withdrawalAddressInput.value === 'YOUR_USDT_TRC20_ADDRESS_NOT_SET') {
                copyWithdrawalFeedback.textContent = 'No address set to copy.';
                copyWithdrawalFeedback.style.color = '#f6465d';
                setTimeout(() => { copyWithdrawalFeedback.textContent = ''; }, 3000);
                return;
            }
            ...
            navigator.clipboard.writeText(withdrawalAddressInput.value).then(() => {
                ...
                setTimeout(() => {
                    ...
                    copyWithdrawalAddressBtn.innerHTML = '<i class="fas fa-copy"></i> Copy';
